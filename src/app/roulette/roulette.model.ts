  ...
  restore(data: Roulette): Roulette {
    const items = Object.keys(data._items).reduce((acc, key) => {
      return Object.assign({}, acc, {
        [key]: RouletteWheelSpin.Build(data._items[key])
      });
    }, {});
...
export class RouletteWheel {
  ...
  static GetCoordinatesRelativeToCenter(
  ...
  static GetDegreesCounterClockwiseFromPosition(
    center: PixelPosition,
    position: PixelPosition
  ): number {
    const x = position.left - center.left;
    const y = position.top - center.top;
    const theta = RouletteWheel.Radians2Degrees(
      Math.atan(Math.abs(y) / Math.abs(x))
    );
    if (x >= 0 && y <= 0) {
      // Q1
      return 270 + theta;
  ...
  static GetQuartileDegrees(startDegrees: number, endDegrees: number): number {
    return RouletteWheel.GetMiddleDegrees(
      startDegrees,
      RouletteWheel.GetMiddleDegrees(startDegrees, endDegrees)
    );
  }
  ...
  static GetPosition(
    ...
    const centralAngle = RouletteWheel.Degrees2CentralAngleClockwise(
      degreesClockwise
    );
    const coordinates = RouletteWheel.GetCoordinatesRelativeToCenter(
      centralAngle,
      distanceFromCenter
    );
    return RouletteWheel.GetPositionRelativeToCenter(
      center,
      coordinates,
      degreesClockwise
    );
  ...
  static GetPositionRelativeToCenter(
    center: PixelPosition,
  ...
  static RotateDegreesClockwise(
    startDegreesClockwise: number,
    rotateDegrees: number
  ): number {
    return (startDegreesClockwise + rotateDegrees) % 360;
  }

  static RotateDegreesCounterClockwise(
    startDegreesClockwise: number,
    rotateDegrees: number
  ): number {
    return RouletteWheel.RotateDegreesClockwise(
      startDegreesClockwise,
      360 - rotateDegrees
    );
  }

  static RotateSlot(
    slot: RouletteWheelSlot,
  ...
  constructor(
    ...
    public ballRevolutionsPerSpin = 8
  ) {
    this.slots = [
  ...
  get ballBeforeSpin(): RouletteBall {
    const position = RouletteWheel.GetPosition(
      this.centerPosition,
      (this.randomSlotNumber * this.slotCentralAngle) % 360,
      this.ballDistanceFromCenterInitial
    );
  ...
  // get ballDegreesTraversedLastMoment(): number {
  //     return this.getBallDegreesTraversedAtTime(this.moment) - this.getBallDegreesTraversedAtTime(Math.max(this.moment - 1, 1));
  // }

  get ballDegreesTraversedPerSpin(): number {
    return this.ballRevolutionsPerSpin * 360;
  }

  get ballDistanceCoveredTotal(): number {
    return this.getBallMotionIntegralAtTime(this.lastMomentForCurrentSpin);
  }

  get ballDistanceGainedPerSpin(): number {
    return (
      this.ballDistanceFromCenterInitial - this.ballDistanceFromCenterFinal
    );
  }

  get ballDistanceGainedLastMoment(): number {
    return (
      this.getBallDistanceGainedAtTime(this.moment) -
      this.getBallDistanceGainedAtTime(Math.max(this.moment - 1, 1))
    );
  }
  ...
  get ballMotionConstant(): number {
    // return 1 * Math.log(this.ballDistanceGainedPerSpin) / (this.totalSpinSeconds * this.momentsPerSecond);
    // return Math.exp(this.ballDistanceGainedPerSpin) / (this.totalSpinSeconds * this.momentsPerSecond);
    return (
      (this.totalSpinSeconds * this.momentsPerSecond) /
      this.ballDistanceGainedPerSpin
    );
  }
  ...
  get lastMomentForCurrentSpin(): number {
    return this.currentSpinStartMoment + this.momentsPerSpin;
  }
  ...
  get nextBallPosition(): PixelPosition {
    return RouletteWheel.GetPositionRelativeToCenter(
      this.centerPosition,
      this.nextBallCoordinatesRelativeToCenter,
      this.nextBallPositionDegreesTraversed
    );
  }

  get nextBallCoordinatesRelativeToCenter(): PixelCoordinates {
    return RouletteWheel.GetCoordinatesRelativeToCenter(
      this.nextBallPositionCentralAngle,
      this.nextBallPositionDistanceFromCenter
    );
  }

  get nextBallPositionDegreesTraversed(): number {
    // TODO: read from ball motion model
    // return (this.moment * this.ballDegreesTraversedLastMoment) % 360;
    return this.getBallDegreesTraversedAtTime(this.moment) % 360;
  }

  get nextBallPositionDistanceFromCenter(): number {
    const d = this.ballDistanceFromCenter - this.ballDistanceGainedLastMoment;
    return Math.max(d, this.ballDistanceFromCenterFinal);
  }

  get nextBallPositionCentralAngle(): number {
    return RouletteWheel.Degrees2CentralAngleClockwise(
      this.nextBallPositionDegreesTraversed
    );
  }
  ...
  get randomSlotString(): string {
    ...
    return r === 38 ? '00' : r === 37 ? '0' : r.toString();
  }
  ...
  getBallDegreesTraversedAtTime(moment: number): number {
    return (
      this.getProportionOfTotalDistanceCoveredAtTime(moment) *
      this.ballDegreesTraversedPerSpin
    );
  }

  getBallDistanceGainedAtTime(moment: number): number {
    const t = this.getMomentsPassedInSpin(moment);
    const k = this.ballMotionConstant;
    // return Math.pow(Math.E, 1 * t * k);
    // return Math.log(k * t);
    return (1 / k) * t;
  }

  getBallMotionIntegralAtTime(moment: number) {
    const t = this.getMomentsPassedInSpin(moment);
    const k = this.ballMotionConstant;
    // return (-1 / k) * Math.pow(Math.E, 1 * t * k);
    // return ((k * t) / k) * Math.log(k * t) - t;
    // return t * Math.log(k * t) - t;
    return ((1 / 2) * k * t) ^ 2;
  }

  getMomentsPassedInSpin(t: number): number {
    return t - this.currentSpinStartMoment + 1;
  }

  getProportionOfTotalDistanceCoveredAtTime(moment: number) {
    return (
      this.getBallMotionIntegralAtTime(moment) / this.ballDistanceCoveredTotal
    );
  }

  getWheelDegreesTraversedAtPosition(pos: PixelPosition): number {
    // WHEEL IS MOVING COUNTER-CLOCKWISE!
  ...
  getDistanceFromCenter(pos: PixelPosition): number {
    return Math.sqrt(
      Math.pow(this.centerPosition.top - pos.top, 2) +
        Math.pow(this.centerPosition.left - pos.left, 2)
    );
  }

  getSlotPosition(slotNumber: string): PixelPosition {
    const slot = RouletteWheel.RotateSlot(
      this.slots.find(x => x.number === slotNumber),
      this.getWheelDegreesTraversedAtTime(this.moment)
    );
  ...
  slotsAtTime(t: number): RouletteWheelSlot[] {
    return RouletteWheel.RotateSlots(
      this.slots,
      this.getWheelDegreesTraversedAtTime(t)
    );
  }
  ...
  startSpin() {
    if (this.started) {
      ...
      this.currentSpinStartMoment = this.moment;
  ...
  endSpin(moment: number, position: PixelPosition) {
    ...
    this.stoppedDegreesCounterClockwise =
      RouletteWheel.GetDegreesCounterClockwiseFromPosition(
        this.centerPosition,
        position
      ) +
      (3 * this.slotCentralAngle) / 4;
  ...
  private getNextBallPositionTransitioning(): PixelPosition {
    ...
    const degreesClockwise = RouletteWheel.RotateDegreesCounterClockwise(
      this.stoppedDegreesClockwise,
      degreesTraversed
    );
    return RouletteWheel.GetPosition(
      this.centerPosition,
      degreesClockwise,
      this.ballDistanceFromCenterStopped
    );
  }
  ...
  private getNextMoment() {
    switch (this.status) {
      ...
      case 'SPINNING':
        if (this.ballStopped) {
          this.endSpin(this.moment - 1, this.ball.position); // TODO: check this logic
        } else {
...
export class RouletteWheelSlot {
  ...
  get ballDegrees(): number {
    // TODO: find correct ball degrees
  ...
  inSlot(degrees: number): boolean {
    return (
      (degrees >= this.startDegrees && degrees < this.endDegrees) ||
      (this.startDegrees > this.endDegrees &&
        (degrees > this.startDegrees || degrees < this.endDegrees))
    );
  }
}
